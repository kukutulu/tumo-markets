...
interface LongShortProps {
  ...
  addPosition: (position: Position, duration: string, amount: number, type: 'Long' | 'Short') => void;
}

const intervals = ['30 Sec', '1 Min', '2 Min', '5 Min'];

export default function TestActionLongShort({
  // balance = 1231234,
  onAmountChange,
  onIntervalChange,
  match_id,
  ...
  const createPositionMutation = useMutation({
    ...
    onSuccess: (response, variables) => {
      // Extract the actual position data from the response
      ...
      // Add position to local state with the user's selected parameters
      addPosition(positionData, variables.duration, variables.amount, variables.type);
    },
