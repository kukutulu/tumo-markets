...
import LiveMatchChart from 'src/views/live-match-detail/LiveMatchChart';
...
import PositionsAndActions from 'src/views/live-match-detail/PositionsAndActions';
...
export default function LiveMatchDetailPage() {
  ...
  const { connected, wallets, select, wallet, connect, connecting, disconnect } = useWallet();
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      try {
        const wsUrl = apiUrl.getMomentumWebSocket(matchId);
  ...
  return (
    <PageTransition direction="forward">
      <div
        className="flex flex-col min-h-screen relative gap-4 px-4 pb-6"
      ...
      >
        ...
        <div className="rounded-2xl bg-[#161616] px-4 py-4 text-white">
        ...
        {activeTab === 0 && (
          <div className="flex flex-col gap-4">
            ...
            {/* Live Momentum Chart */}
            <LiveMatchChart momentumData={momentumData} theme={theme} />
          </div>
        )}
        ...
        />
        {/* {connected ? (
          // Pass the actual live time (use `time` which reads from match or websocket-updated value)
          <ActionLongShort match_id={matchId} entry={currentMomentum} time_in_match={time} />
        ) : (
          <button
            onClick={() => setShowWallets(true)}
            disabled={connecting}
            className="w-full rounded-lg bg-linear-to-r from-[#1c54ff] to-[#001a61] dark:from-[#e4e9ff] dark:to-[#4c5061] hover:from-[#163fbf] hover:to-[#001244] text-white dark:text-black font-semibold py-3 px-6 shadow-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {connecting ? (
              <>
                <RotateHourGlass />
